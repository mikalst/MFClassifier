...
def confusion_matrix(model, X_and_t, y_true):
    predicted_z = model.predict(X_and_t)

    return sklearn.metrics.confusion_matrix(y_true, predicted_z)


def confusion_matrix_bin(model, X_and_t, y_true):
    predict_bin = model.predict_binary(X_and_t)
    y_true_bin = model.z_to_binary_mapping(y_true)

    return sklearn.metrics.confusion_matrix(y_true_bin, predict_bin)


def accuracy_score(model, X_and_t, y_true):
    # Classification full
    predicted_z = model.predict(X_and_t)
...
def accuracy_event_score(model, X_and_t, y_true):
    predicted_e = model.predict_event(X_and_t)

    return np.mean(predicted_e == list(map(model.z_to_e_mapping, y_true)))


def sensitivity_score(model, X_and_t, y_true):
    predicted_bin = model.predict_binary(X_and_t)
    y_true_bin = model.z_to_binary_mapping(y_true)
...
def specificity_score(model, X_and_t, y_true):
    predicted_bin = model.predict_binary(X_and_t)
    y_true_bin = model.z_to_binary_mapping(y_true)
...
def mcc_score(model, X_and_t, y_true):
    predicted_bin = model.predict_binary(X_and_t)
    y_true_bin = model.z_to_binary_mapping(y_true)
