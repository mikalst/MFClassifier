...
def steepest_descent(f, g, x0, TOL=1e-3, max_iter=99, verbosity=False):
    ...
    if verbosity >= 1:
        print(
            'SD j:{:.3e} -> {:.3e} in {} iterations.'.format(f(x0), f_final, iterations))
...
def bfgs(f, g, x0, TOL=1e-3, max_iter=99, linesearch_method="ww", verbosity=0):
    ...
    while True:
        ...
        if linesearch_method == "bt":
            x_k, ls_success = backtracking_linesearch(
                f, g, x_k, p_k, g_k, verbosity=verbosity)

        elif linesearch_method == "ww":
            x_k, ls_success = linesearch(
                f, g, x_k, p_k, 1e-4, 0.9, wolfe='w', verbosity=verbosity)
        ...
        if (y.dot(s) <= 0):
            ...
            rho = 1/y.dot(s)
            H_k = (I - rho*np.outer(s, y))@H_k@(I - rho *
    ...
    if verbosity >= 1:
        print(
            'BFGS j:{:.3e} -> {:.3e} in {} iterations.'.format(f(x0), f_final, iterations))
...
def fletcher_reeves(f, g, x0, TOL=1e-3, max_iter=99, verbosity=0):
    ...
    while True:
        ...
        x_k, ls_success = linesearch(
            f, g, x_k, p_k, 1e-4, 0.90, wolfe='s', verbosity=verbosity)
    ...
    if verbosity >= 1:
        print(
            'FR j:{:.3e} -> {:.3e} in {} iterations.'.format(f(x0), f_final, iterations))
