...
def simulate_mask(
    ...
    individual_probs = np.empty((X.shape[0], mask_transition_expectations.shape[0]))
    ...
    for t in range(X.shape[1] - 1):
        ...
        last_remembered_values = observed_values[np.arange(observed_values.shape[0]), t+1-np.argmax(observed_values[:, t+1:max(0, t-mask_memory_length):-1] != 0, axis=1)]

        indices = ((np.arange(np.max(X)+1))[:, None] == last_remembered_values).T
...
def simulate_ordinal_from_float(
    ...
    weights_normalized = weights# / np.reshape(np.sqrt((weights**2).sum(axis=1)), (38001, 1))
    ...
    for j in range(1, matrix.shape[1]):
        ...
        d = distribution(column_repeated, domain_repeated) * neighbours * original_data_pdf
        cdf = np.cumsum(d / np.reshape(np.sum(d, axis=1), (matrix.shape[0], 1)), axis=1)
